...
const autoProvision = async () => {
  try {
    ...
    if (userCount > 0) {
      console.log('Database already provisioned. Skipping auto-seed.');
      return;
    }
    console.log('Empty Database detected! Automatically provisioning essential Settings and Admin User...');
    ...
    console.log('Auto Provisioning Success!');
  } catch (error) {
    console.error(`Error during auto-provisioning: ${error.message}`);
  }
};
